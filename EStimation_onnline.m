...
for k=1:length(t)-1
    ...
    if sample > 1
        estados = [v_estimate1(:,k:k+1);vref(:,k-1:k)];
        U = estados;
        V = v(:,k:k+1);
        C = -1*eye(2);
        C(end,end) = 1;
        factor = inv(inv(C)+(U'*P_k*U));
        P_k = P_k - P_k*(U*factor*U')*P_k;
        G_k = G_k+(V-G_k*U)*factor*U'*P_k;
        A_k = G_k(:,1:4);
        B_k = G_k(:,5:end);
        sample = 0;
...
plot(t,v(1,:),'--','Color',[226,76,44]/255,'linewidth',1); hold on
...
plot(t,v(2,:),'--','Color',[46,188,89]/255,'linewidth',1); hold on
...
plot(t,v(3,:),'--','Color',[26,115,160]/255,'linewidth',1); hold on
...
plot(t,v(4,:),'--','Color',[83,57,217]/255,'linewidth',1); hold on
