...
for k=1:length(t)
    ...
    v_estimate2(:, k+1) = system_dynamic(x, v_estimate2(:,k), v_ref(:,k), ts);
    
end

%% Definition of System Matrcies
A = [-0.5827   -0.0721    0.0587   -0.0280;...
    0.0214   -0.4770    0.0731   -0.1840;...
    0.0152    0.0102   -3.5696    0.0064;...
    0.0377   -0.0113   -0.0058   -8.2424];

B = [0.8048   -0.0376   -0.0793   -0.1797;...
    0.0634    0.8527   -0.0783    0.3821;...
   -0.0202   -0.0046    3.5776    0.0607;...
   -0.0251   -0.0029   -0.0288    8.1294];
%% SIMULATION DYNAMICS
v_estimate = v(:,1);
for k=1:length(t)
...
ERROR = [ul_err_op, ul_err_dmd;...
         um_err_op, um_err_dmd;...
         un_err_op, un_err_dmd;...
         w_err_op, w_err_dmd];
...
plot(t,v_estimate2(1,1:length(t)),'--','Color',C12,'LineWidth',lw);
grid minor;
grid minor;
...
%plot(t,ul,'-','Color',C2,'LineWidth',lw);
plot(t,v_estimate2(2,1:length(t)),'--','Color',C14,'LineWidth',lw);
grid minor;
...
plot(t,v_estimate2(3,1:length(t)),'--','Color',C15,'LineWidth',lw);
%plot(t,ul,'-','Color',C2,'LineWidth',lw);
grid minor;
...
plot(t,v_estimate2(4,1:length(t)),'--','Color',C17,'LineWidth',lw);
%plot(t,ul,'-','Color',C2,'LineWidth',lw);
grid minor;
...
%s = reordercats(s,{'$\it{Label\ 1}$', '$\pi r^2$', '$\textbf{Label\ 3}$', '$\frac{foo}{bar}dy$'});
figure('Position', [10 10 sizeX sizeY])
set(gcf, 'PaperUnits', 'inches');
set(gcf, 'PaperSize', [8.5 11]);
set(gcf, 'PaperPositionMode', 'manual');
...
%title({'(a)'},'fontsize',fontsizeTitel,'interpreter','latex')
% set(gca,'Xticklabel',[])
legend({'$\textrm{SQP}$','$\textrm{DMD}$'},'interpreter','latex','fontsize',fontsizeLegend)
